...
namespace util2 {
	...
	public:
		...
		int countBuses(util::StaticVector<util::StaticVector<int, 2>, MAX_BUS_NUM>& buses, int time) {
			int checkCount{ 0 };
		...
		int checkDifferentPeriod(util::StaticVector<util::StaticVector<int, 2>, MAX_BUS_NUM>& buses, int time) {
			for (std::size_t i = 0; i < buses.getSize() - 1; ++i) {
				for (std::size_t j = 0; j < buses[i].getSize(); ++j) {
					if (time % buses[i][j] == 0)
					return buses[i][j];
		...
		void editIntervals(util::StaticVector<util::StaticVector<int, 2>, MAX_BUS_NUM>& buses, int time, int num) {
			for (std::size_t i = 0; i < buses.getSize() - 1; ++i) {
				for (std::size_t j = 0; j < buses[i].getSize(); ++j) {
					if (time % buses[i][j] == 0) {
						if (num == -1)
		...
		void printBuses(util::StaticVector<util::StaticVector<int, 2>, MAX_BUS_NUM>& buses) {
			for (std::size_t i = 0; i < buses.getSize(); ++i) {
				for (std::size_t j = 0; j < buses[i].getSize(); ++j) {
					std::cout << buses[i][j];
				}
		...
		void FindInterval(util::StaticVector<util::StaticVector<int, 2>, MAX_BUS_NUM>& allIntervals) {
			...
			for (std::size_t i = 0; i < allTokens.getSize(); ++i) {
				...
				if (check_count - 1 == stop_count) {
					...
					diffPeriodCount++; // 15->30 olunca bus yok yani 15'in periodu deðiþiyor
